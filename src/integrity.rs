...
//
// Copyright 2018 Andy Grover <andy@groveronline.com>
//
...
impl IntegrityDev {
    ...
    pub fn initialize(
        ...
        idev.table_load(dm, &table, DmOptions::default())?;
        idev.resume(dm)?;
        idev.table = table;
    ...
    pub fn status(&self, dm: &DM) -> DmResult<IntegrityDevStatus> {
        let (_, status) = dm.table_status(&DevId::Name(self.name()), DmOptions::default())?;
...
pub struct IntegrityDevStatus {
    pub mismatches: u64,
    pub provided_sectors: Sectors,
    pub recalc_sector: Option<Sectors>,
}
...
fn write_sectors<P: AsRef<Path>>(
    ...
    f.seek(SeekFrom::Start(*offset.bytes() as u64))?;
    for _ in 0..*length {
        f.write_all(buf)?;
    }
